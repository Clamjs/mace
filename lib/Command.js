...
util.inherits(Command, {
  ...
  option: function (flag, description, handle) {
    var opt = new Option(flag, description, handle);
  ...
  parse: function (info) {
    ...
    var cmd = command;
    var params = {};
    util.each(info.options, function (val, key) {
      var opt;
      while(cmd && !opt) {
        var opts = cmd._options;
        var alias = cmd._alias;
        if (alias[key]) {
          key = alias[key];
        }
        if (opts[key]) {
