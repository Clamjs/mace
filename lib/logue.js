...
function _log (name, conf) {
  ...
  var format = conf.format || color.format;
  ...
  var output = function () {
    ...
    var ret = tab + split;
    var content = format.apply(null, arguments);
    if (stamp) {
      ret += color.styles.black(lang.stamp(new Date, stamp)) + split;
    }
    ret += name + split + content;
...
util.color = color;
util.speclog = function (name, conf) {
  return _log(name, conf);
};
util.line = _line(1);
util._line = _line;
util.stringify = color.stringify;
util.colorfull = color.format;
...
util.debug = (function (models) {
  ...
  function debug (ns, disabled) {
    ...
    var fn = models[id] = models[id] || _log(getName(ns), {
      ...
      color: _color
      //stamp  时间戳 
      // full format: `tab - stamp - name - message - time - line`
    });
